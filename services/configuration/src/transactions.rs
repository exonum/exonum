...
fn validator_index(snapshot: &DbView, key: &PublicKey) -> Option<usize> {
...
fn enough_votes_to_commit(snapshot: &DbView, cfg_hash: &Hash) -> bool {
    let actual_config = CoreSchema::new(snapshot).actual_configuration();
...
impl Propose {
    ...
    fn precheck(&self, snapshot: &DbView) -> Result<(StoredConfiguration, Hash), ServiceError> {
...
impl<'a> VotingDecisionRef<'a> {
    ...
    fn precheck(&self, snapshot: &DbView) -> Result<StoredConfiguration, ServiceError> {
        use self::ServiceError::*;
