use exonum::crypto::{hash, gen_keypair, Hash};
...
fn test_modify_user_by_self() {
    ...
    let tx1 = TxUpdateUser::new(&keypair1.0, user_info1.clone(), &keypair1.1);
    let tx2 = TxUpdateUser::new(&keypair1.0, user_info2.clone(), &keypair1.1);
...
fn test_modify_user_by_other() {
    ...
    let tx1 = TxUpdateUser::new(&keypair1.0, user_info1.clone(), &keypair1.1);
    let tx2 = TxUpdateUser::new(&keypair2.0, user_info2.clone(), &keypair2.1);
    testkit.create_block_with_transactions(txvec![tx1, tx2]);
    // check result
    let schema = Schema::new(testkit.snapshot());
    let user_id_hash = schema.users_history().get(0).unwrap();
...
fn test_add_payment_from_billing() {
    ...
    let tx2 = TxPayment::new(&payment_keypair.0, payment_info, &payment_keypair.1);
    testkit.create_block_with_transactions(txvec![tx1, tx2]);
    // check result
    let schema = Schema::new(testkit.snapshot());
    let user_id_hash = schema.users_history().get(0).unwrap();
...
fn test_timestamp_by_other() {
    ...
    let timestamp = Timestamp::new("first_user", &hash(&[1, 2, 4]), "metadata");
    let tx1 = TxUpdateUser::new(&keypair.0, user_info.clone(), &keypair.1);
    let tx2 = TxTimestamp::new(&keypair_other.0, timestamp.clone(), &keypair_other.1);
    testkit.create_block_with_transactions(txvec![tx1, tx2]);
    // check result
    let schema = Schema::new(testkit.snapshot());
    let user_id_hash = schema.users_history().get(0).unwrap();
...
fn test_timestamp_by_owner_with_old_key() {
    ...
    let timestamp = Timestamp::new("first_user", &hash(&[1, 2, 4]), "metadata");
    let tx1 = TxUpdateUser::new(&keypair_old.0, user_info_old.clone(), &keypair_old.1);
    let tx2 = TxUpdateUser::new(&keypair_old.0, user_info.clone(), &keypair_old.1);
    let tx3 = TxTimestamp::new(&keypair_old.0, timestamp.clone(), &keypair_old.1);
    // check result
...
fn test_timestamp_exists_content_hash() {
    ...
    let tx1 = TxUpdateUser::new(&keypair1.0, user_info1.clone(), &keypair1.1);
    let tx2 = TxUpdateUser::new(&keypair2.0, user_info2.clone(), &keypair2.1);
    let tx3 = TxTimestamp::new(&keypair1.0, timestamp1.clone(), &keypair1.1);
    let tx4 = TxTimestamp::new(&keypair2.0, timestamp2.clone(), &keypair2.1);
