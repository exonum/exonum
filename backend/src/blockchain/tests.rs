...
impl TimestampingBlockchain {
    fn new() -> TimestampingBlockchain {
        ...
        let inner = Blockchain::new(Box::new(db), vec![Box::new(timestamping_service)]);
...
#[test]
fn test_add_user() {
    let mut blockchain = TimestampingBlockchain::new();
    ...
    blockchain.execute_transaction(TxUpdateUser::new(&keypair.0, user_info.clone(), &keypair.1));
    // check result
...
fn test_modify_user_by_self() {
    ...
    blockchain.execute_transaction(TxUpdateUser::new(
        &keypair1.0,
        user_info1.clone(),
        &keypair1.1,
    ));
...
fn test_add_payment_from_billing() {
    ...
    let payment_keypair = blockchain.payment_keypair();
    blockchain.execute_transaction(TxUpdateUser::new(&keypair.0, user_info.clone(), &keypair.1));
    blockchain.execute_transaction(TxPayment::new(
        &payment_keypair.0,
        payment_info,
        &payment_keypair.1,
    ));
...
fn test_timestamp_by_owner_with_actual_key() {
    ...
    blockchain.execute_transaction(TxUpdateUser::new(&keypair.0, user_info.clone(), &keypair.1));
    blockchain.execute_transaction(TxTimestamp::new(&keypair.0, timestamp.clone(), &keypair.1));
    // check result
    let schema = Schema::new(blockchain.snapshot());
    let user_id_hash = schema.users_history().get(0).unwrap();
    let user_entry = schema.users().get(&user_id_hash).unwrap();
