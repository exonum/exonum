...
use super::config::{LoggerOption, OutputConfig,
                    ColorConfig, TimestampConfig, FormatConfig};

lazy_static! {
    static ref INIT_TIMER: SystemTime = SystemTime::now();
}

pub(crate) fn build_drain(opts: &LoggerOption)
...
fn color_terminal(builder: TermDecoratorBuilder,
                  opts: &OutputConfig) -> TermDecoratorBuilder {
     match opts.color {
...
fn full_terminal(builder: TermDecoratorBuilder,
                 opts: &OutputConfig) -> Box<Drain<Ok = (), Err = Never> + Send> {
...
fn compact_terminal(builder: TermDecoratorBuilder,
                    opts: &OutputConfig) -> Box<Drain<Ok = (), Err = Never> + Send> {
...
fn make_json<W: io::Write + Send + 'static>(w: W, opts: &OutputConfig) -> Box<Drain<Ok = (), Err = Never> + Send> {

    let json = Json::new(w);
    let json = match opts.timestamp {
        TimestampConfig::None => json,
        TimestampConfig::Utc => json.add_key_value(o!("ts" => FnValue(move |_| {
                                                            timestamp_term_to_json(::slog_term::timestamp_utc)
                                                        }))),
        TimestampConfig::Local => json.add_key_value(o!("ts" => FnValue(move |_| {
                                                            timestamp_term_to_json(::slog_term::timestamp_local)
                                                        }))),
        TimestampConfig::Ticks => json.add_key_value(o!("ts" => FnValue(move |_| {
...
fn tick_timestamp(w: &mut Write) -> ::std::io::Result<()> {
    match INIT_TIMER.elapsed()
        {
            Ok(d) => w.write_fmt(format_args!("{}.{:03}",
                                              d.as_secs(),
...
fn nop_timestamp(_: &mut Write) -> ::std::io::Result<()> {Ok(())}
