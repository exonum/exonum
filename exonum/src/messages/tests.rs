...
use crate::crypto::{gen_keypair, hash, CryptoHash, PublicKey, SecretKey};
...
fn test_message_roundtrip() {
    ...
    let status = Status::new(Height(2), &hash(&[]));

    let msg = Message::concrete(status.clone(), pub_key, &secret_key);

    let bytes = msg.encode().expect("Encode error.");
    SignedMessage::decode(&bytes).expect("Sign error.");
    //    let msg_roundtrip = Message::deserialize(
    //        SignedMessage::decode(&bytes).expect("Sign decode error"))
    //            .expect("MEssage des error");
}
