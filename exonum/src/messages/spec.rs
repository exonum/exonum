...
//                $(
//                    let $field_name: $field_type = unsafe { <$field_type as Field>::from_value(_obj.remove(stringify!($field_name)).unwrap()) };
//                    println!("{}: {} = {:?}", stringify!($field_name), stringify!($field_type), $field_name);
//                )*
//
////                let mut buf = vec![0 as u8;<&Signature as Field>::field_size() as usize];
////                <&Signature as FromValue>::from_value(obj.remove(&"signature".to_string()).unwrap(), &mut buf);
//                let signature: &Signature = unsafe { <&Signature as Field>::from_value(_obj.remove("signature").unwrap()) };
//
//                Ok(Self::new_with_signature($($field_name,)* signature))
            }
        }
