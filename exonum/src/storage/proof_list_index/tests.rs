...
fn list_methods(db: Box<Database>) {
    let fork = db.fork();
    let mut index = ProofListIndex::new("a1", fork);
...
fn height(db: Box<Database>) {
    let fork = db.fork();
    let mut index = ProofListIndex::new("a2", fork);
...
fn iter(db: Box<Database>) {
    let fork = db.fork();
    let mut index = ProofListIndex::new("a3", fork);
    index.extend(vec![1u8, 2, 3]);

    assert_eq!(index.len(), 3);
    //    assert_eq!(index.iter().collect::<Vec<u8>>(), vec![1, 2, 3]);
    //    assert_eq!(index.iter_from(0).collect::<Vec<u8>>(), vec![1, 2, 3]);
    assert_eq!(index.iter_from(1).collect::<Vec<u8>>(), vec![2, 3]);
    //    assert_eq!(
    //        index.iter_from(3).collect::<Vec<u8>>(),
    //        Vec::<u8>::new()
    //    );
}
...
fn list_index_proof(db: Box<Database>) {
    let fork = db.fork();
    let mut index = ProofListIndex::new("a4", fork);
...
fn randomly_generate_proofs(db: Box<Database>) {
    let fork = db.fork();
    let mut index = ProofListIndex::new("a5", fork);
    let num_vals = 100;
...
fn index_and_proof_roots(db: Box<Database>) {
    let fork = db.fork();
    let mut index = ProofListIndex::new("a6", fork);
...
fn proof_illegal_lower_bound(db: Box<Database>) {
    let fork = db.fork();
    let mut index = ProofListIndex::new("a7", fork);
...
fn proof_illegal_bound_empty(db: Box<Database>) {
    let fork = db.fork();
    let mut index = ProofListIndex::new("a8", fork);
    for i in 0u8..8 {
        index.push(vec![i]);
    }
...
fn proof_illegal_range(db: Box<Database>) {
    let fork = db.fork();
    let mut index = ProofListIndex::new("a9", fork);
    for i in 0u8..4 {
        index.push(vec![i]);
    }
...
fn proof_structure(db: Box<Database>) {
    let fork = db.fork();
    let mut index = ProofListIndex::new("a10", fork);
    assert_eq!(index.root_hash(), Hash::zero());
...
fn simple_root_hash(db: Box<Database>) {
    ...
    let fork = db.fork();
    let mut index = ProofListIndex::new("a11", fork);
...
fn same_root_hash(db1: Box<Database>, db2: Box<Database>) {
    let fork1 = db1.fork();
    let mut i1 = ProofListIndex::new("a12", fork1);
    i1.push(vec![1]);
    i1.push(vec![2]);
    i1.push(vec![3]);
    i1.push(vec![4]);
    ...
    let fork2 = db2.fork();
    let mut i2 = ProofListIndex::new("a13", fork2);
    i2.push(vec![4]);
    i2.push(vec![7]);
    i2.push(vec![5]);
    i2.push(vec![1]);
