...
proptest! {
    #[test]
    fn proptest_value_set_index_to_rust_set(ref actions in vec(generate_action(),
                                                             1..ACTIONS_MAX_LEN)) {
        let db = MemoryDB::new();
        ...
        for action in actions {
            match action {
                ...
                _ => {
                    let mut set_index = ValueSetIndex::new("test", &mut fork);
        ...
        let mut fork = db.fork();
        let set_index = ValueSetIndex::new("test", &mut fork);
        compare_collections(&set_index, &ref_set)?;
    }
}
