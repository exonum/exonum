...
```
На данный момент гарантировано все работает на nightly версии от 19 декабря, для того,
чтобы перейти на ее использование, нужно в корне проекта выполнить команду:
```
rustup override set nightly-2016-12-19
```
...
### FIXME полностью переписать этот раздел, а пока проще посмотреть в .gitlab-ci.yml
