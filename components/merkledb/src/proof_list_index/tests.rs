...
fn list_methods() {
    ...
    let fork = db.fork();
    let mut index = fork.as_ref().get_proof_list(IDX_NAME);
...
fn iter() {
    let db = TemporaryDB::new();
    let fork = db.fork();
    let mut list_index = fork.as_ref().get_proof_list(IDX_NAME);
...
fn proofs_in_empty_list() {
    let db = TemporaryDB::new();
    let fork = db.fork();
    let index: ProofListIndex<_, u32> = fork.as_ref().get_proof_list(IDX_NAME);
...
fn setting_elements_leads_to_correct_list_hash() {
    ...
    let hash1 = {
        let fork = db.fork();
        let mut list = fork.as_ref().get_proof_list(IDX_NAME);
        list.push(vec![1]);
        list.push(vec![2]);
        list.push(vec![3]);
        list.push(vec![4]);
...
mod root_hash {
        ...
        V: BinaryValue + Clone,
    {
        let db = TemporaryDB::new();
        let fork = db.fork();
        let mut index = fork.as_ref().get_proof_list("merkle_root");
