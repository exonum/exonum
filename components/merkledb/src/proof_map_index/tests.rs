...
fn test_map_methods() {
    ...
    let mut index = fork.as_ref().ensure_proof_map(IDX_NAME);
...
fn test_insert_trivial() {
    ...
    let mut index1 = fork1.as_ref().ensure_proof_map(IDX_NAME);
    index1.put(&[255; 32], vec![1]);
    index1.put(&[254; 32], vec![2]);

    let mut index2 = fork2.as_ref().ensure_proof_map(IDX_NAME);
...
fn test_insert_same_key() {
    let db = TemporaryDB::new();
    let fork = db.fork();
    let mut table = fork.as_ref().ensure_proof_map(IDX_NAME);
...
fn test_iter() {
    let db = TemporaryDB::new();
    let fork = db.fork();
    let mut map_index = fork.as_ref().ensure_proof_map(IDX_NAME);
