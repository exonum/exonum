...
fn plain_map_index_insert(b: &mut Bencher, len: usize) {
    ...
    b.iter_with_setup(
        ...
        |(db, data)| {
            let fork = db.fork();
            {
                let mut table = fork.as_ref().get_map(NAME);
                for item in data {
...
fn plain_map_index_with_family_insert(b: &mut Bencher, len: usize) {
    ...
    b.iter_with_setup(
        ...
        |(db, data)| {
            let fork = db.fork();
            {
                let mut table = fork.as_ref().get_map((NAME, FAMILY));
                for item in data {
                    table.put(&item.0, item.1);
                }
            }
...
fn plain_map_index_iter(b: &mut Bencher, len: usize) {
    ...
    b.iter_with_setup(
        || db.snapshot(),
        |snapshot| {
            let index: MapIndex<_, Hash, Vec<u8>> = snapshot.as_ref().get_map(NAME);
...
fn plain_map_index_with_family_iter(b: &mut Bencher, len: usize) {
    ...
    b.iter_with_setup(
        || db.snapshot(),
        |snapshot| {
            let index: MapIndex<_, Hash, Vec<u8>> = snapshot.as_ref().get_map((NAME, FAMILY));
            for (key, value) in &index {
                black_box(key);
                black_box(value);
            }
...
fn proof_list_append(b: &mut Bencher, len: usize) {
    ...
    b.iter_with_setup(
        || (db.fork(), data.clone()),
        |(fork, data)| {
            let mut table = fork.as_ref().get_proof_list(NAME);
...
fn proof_map_insert_without_merge(b: &mut Bencher, len: usize) {
    ...
    b.iter_with_setup(
        || (db.fork(), data.clone()),
        |(fork, data)| {
            let mut table = fork.as_ref().get_proof_map(NAME);
            for item in data {
                table.put(&item.0, item.1);
            }
        },
