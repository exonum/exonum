...
// TODO reimplement tests with a new business logic. [ECR-3266]
...
// use exonum_testkit::{txvec, TestKit, TestKitBuilder, TestNode};
...
//         // Push votes
//         let cfg_proposal_hash = cfg_proposal.object_hash();
//         let tx_votes = self
//             .network()
//             .validators()
//             .iter()
...
//     assert!(testkit.find_propose(new_cfg.object_hash()).is_none());
// }
...
//     assert_eq!(
//         Some(propose_tx),
//         testkit.find_propose(new_cfg.object_hash())
//     );
// }
...
//     let legal_vote = new_tx_config_vote(&testkit.network().validators()[3], new_cfg.object_hash());
//     let illegal_vote =
//         new_tx_config_vote(&testkit.network().validators()[3], absent_cfg.object_hash());
//     testkit.create_block_with_transactions(txvec![legal_vote.clone(), illegal_vote.clone()]);

//     let votes = testkit.votes_for_propose(new_cfg.object_hash());
//     assert!(votes.contains(&Some(VotingDecision::Yea(legal_vote.object_hash()))));
//     assert!(!votes.contains(&Some(VotingDecision::Yea(illegal_vote.object_hash()))));
// }
...
//     let legal_vote =
//         new_tx_config_vote_against(&testkit.network().validators()[3], new_cfg.object_hash());
//     let illegal_vote =
//         new_tx_config_vote(&testkit.network().validators()[3], new_cfg.object_hash());
//     testkit.create_block_with_transactions(txvec![legal_vote.clone(), illegal_vote.clone()]);

//     let votes = testkit.votes_for_propose(new_cfg.object_hash());
//     assert!(votes.contains(&Some(VotingDecision::Nay(legal_vote.object_hash()))));
//     assert!(!votes.contains(&Some(VotingDecision::Yea(illegal_vote.object_hash()))));
// }
...
//     let propose_tx = new_tx_config_propose(&testkit.network().validators()[1], new_cfg.clone());
//     testkit.create_block_with_transactions(txvec![propose_tx]);
//     assert_eq!(None, testkit.find_propose(new_cfg.object_hash()));
// }
...
//         let validators = testkit.network().validators();
//         txvec![
//             new_tx_config_vote(&validators[1], new_cfg.object_hash()),
//             new_tx_config_vote(&validators[3], new_cfg.object_hash()),
//         ]
//     };
//     testkit.create_block_with_transactions(legal_votes);
//     testkit.create_blocks_until(Height(10));
//     let illegal_vote =
//         new_tx_config_vote(&testkit.network().validators()[0], new_cfg.object_hash());
//     testkit.create_block_with_transactions(txvec![illegal_vote.clone()]);
//     assert!(!testkit
//         .votes_for_propose(new_cfg.object_hash())
//         .contains(&Some(VotingDecision::Yea(illegal_vote.object_hash()))));
// }
...
//     let cfg_proposal_hash = first_cfg.object_hash();
...
//     testkit.create_block_with_transactions(txvec![tx_propose]);
//     assert!(testkit.find_propose(second_cfg.object_hash()).is_none());
//     assert!(testkit.find_propose(first_cfg.object_hash()).is_some());
// }
...
//         assert!(testkit
//             .find_propose(new_cfg_bad_previous_cfg.object_hash())
//             .is_none());
//         assert!(testkit.find_propose(new_cfg.object_hash()).is_none());
//     }
//     {
...
//         assert!(testkit
//             .find_propose(discarded_votes_cfg.object_hash())
//             .is_some());
//         assert!(testkit.find_propose(new_cfg.object_hash()).is_some());
//     }
//     {
//         let illegal_node = TestNode::new_auditor();
//         let illegal_validator_vote =
//             new_tx_config_vote(&illegal_node, discarded_votes_cfg.object_hash());
//         testkit.create_block_with_transactions(txvec![illegal_validator_vote.clone()]);
//         assert!(!testkit
//             .votes_for_propose(discarded_votes_cfg.object_hash())
//             .contains(&Some(VotingDecision::Yea(
//                 illegal_validator_vote.object_hash()
//             ))))
//     }
//     {
//         let votes = (0..3)
//             .map(|id| {
//                 new_tx_config_vote(&testkit.network().validators()[id], new_cfg.object_hash())
//             })
//             .collect::<Vec<_>>();
...
//                     discarded_votes_cfg.object_hash(),
//                 )
//             })
//             .collect::<Vec<_>>();
//         testkit.create_block_with_transactions(expected_votes.clone().into_iter());

//         let actual_votes = testkit.votes_for_propose(discarded_votes_cfg.object_hash());
//         for expected_vote in expected_votes {
//             assert!(!actual_votes.contains(&Some(VotingDecision::Yea(expected_vote.object_hash()))));
//         }
//     }
// }
...
//                 new_tx_config_vote(validator, new_cfg1.object_hash())
//             })
//             .collect::<Vec<_>>();
//         testkit.create_block_with_transactions(votes);
//         assert_eq!(
...
//             new_tx_config_vote(&testkit.network().validators()[2], new_cfg2.object_hash());
...
//             new_tx_config_vote(&testkit.network().validators()[2], new_cfg1.object_hash());
...
//         cfg.set_service_config("dummy", "Second cfg");
//         let mut stored = cfg.stored_configuration().clone();
//         stored.previous_cfg_hash = new_cfg1.object_hash();
//         stored
//     };
//     {
//         let propose_tx1 =
...
//             .map(|validator| {
//                 let validator = &testkit.network().validators()[validator];
//                 new_tx_config_vote(validator, new_cfg2.object_hash())
//             })
//             .collect::<Vec<_>>();
//         testkit.create_block_with_transactions(votes);
